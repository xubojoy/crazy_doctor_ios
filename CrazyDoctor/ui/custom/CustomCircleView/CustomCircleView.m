...
- (void)configLbl {
    ...
    lbl.font = [UIFont systemFontOfSize:default_1_font_size];
    NSString *realStepStr= [NSString stringWithFormat:@"%ld步",(long)self.realStep];
    int start = (int)[realStepStr rangeOfString:[NSString stringWithFormat:@"%ld",(long)self.realStep]].location;
    NSLog(@">>>>>>>>>>开始位置>>>>>>>>>%d",start);
    int length = (int)[realStepStr rangeOfString:[NSString stringWithFormat:@"%ld",(long)self.realStep]].length;
    NSMutableAttributedString *attributedText = [[NSMutableAttributedString alloc] initWithString:realStepStr];
    [attributedText addAttribute:NSFontAttributeName value:[UIFont systemFontOfSize:31] range:NSMakeRange(start,length)];
