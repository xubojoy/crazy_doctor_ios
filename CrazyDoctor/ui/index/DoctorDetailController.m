...
- (void)viewDidLoad {
    ...
    [self initHeadView];
    [self loadWebView];
    NSLog(@">>>>self.articleId>>>>%d",self.doctorId);
}

//初始化自定义导航
-(void)initHeadView{
    self.headerView = [[HeaderView alloc] initWithTitle:@"医生详情" navigationController:self.navigationController];
...
-(void)loadWebView{
    ...
    NSURL *nsurl =[NSURL URLWithString:[NSString stringWithFormat:@"%@/doctors/%d?noFooterFlag=0&noHeaderFlag=0&isAppOpen=1&Authorization=%@&longitude=%f&latitude=%f",[AppStatus sharedInstance].wxpubUrl,self.doctorId,as.user.accessToken,as.lastLng,as.lastLat]];
