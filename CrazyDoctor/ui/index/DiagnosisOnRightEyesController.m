//
//  DiagnosisOnRightEyesController.m
//  CrazyDoctor
//
...
@interface DiagnosisOnRightEyesController ()

@end
static NSString *diagnosisOnEyesCellIdentifier1 = @"diagnosisOnEyesCell";
static NSString *diagnosisOnEyesSelectCollectionCellId1 = @"diagnosisOnEyesSelectCollectionCell";
@implementation DiagnosisOnRightEyesController
- (instancetype)initWithSelectLeftIdArray:(NSMutableArray *)pointIdArray{
    self = [super init];
    if (self) {
        self.pointIdArray = pointIdArray;
    }
    return self;
}
- (void)viewDidLoad {
...
- (void)laodData{
    [EyesDiagnosisStore getUserEyesDiagnoses:^(NSDictionary *eyesDiagnoseDic, NSError *err) {
        if (eyesDiagnoseDic != nil) {
            NSArray *leftArray = eyesDiagnoseDic[@"right"];
...
//初始化自定义导航
-(void)initHeadView{
    self.headerView = [[HeaderView alloc] initWithTitle:@"协助眼诊-右" navigationController:self.navigationController];
    [self.view addSubview:self.headerView];
}

//初始化tableview
-(void)initTableView{
    self.tableView = [[UITableView alloc] initWithFrame:CGRectMake(0, self.headerView.frame.size.height+splite_line_height, screen_width,screen_height-tabbar_height-50-self.headerView.frame.size.height) style:UITableViewStylePlain];
    ...
    UINib *nib = [UINib nibWithNibName:@"DiagnosisOnEyesCell" bundle:nil];
    [self.tableView registerNib:nib forCellReuseIdentifier:diagnosisOnEyesCellIdentifier1];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    if (indexPath.row == 0) {
        ...
        UITableViewCell *cell = [UITableViewCell new];
        cell.selectionStyle = UITableViewCellSeparatorStyleNone;
        self.rightEyeImgView = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, screen_width, (screen_width*256)/375)];
        
        self.rightEyeImgView.image = [UIImage imageNamed:@"icon_eye_frame"];
        [cell.contentView addSubview:self.rightEyeImgView];
...
//        DiagnosisOnEyesCell *cell = [tableView dequeueReusableCellWithIdentifier:diagnosisOnEyesCellIdentifier1 forIndexPath:indexPath];
//        cell.selectionStyle = UITableViewCellSeparatorStyleNone;
...
- (void)initDiagnosisEyesCollectionView{
    ...
    if (row == 1) {
        ...
        self.tableView.frame = CGRectMake(0, self.headerView.frame.size.height+splite_line_height, screen_width, screen_height-51-50-10-10-self.headerView.frame.size.height);
        ...
        self.tableView.frame = CGRectMake(0, self.headerView.frame.size.height+splite_line_height, screen_width, screen_height-51-76-10-self.headerView.frame.size.height);
    ...
    [self.collectionView registerNib:nib
          forCellWithReuseIdentifier:diagnosisOnEyesSelectCollectionCellId1];
...
- (UICollectionViewCell*)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    DiagnosisOnEyesSelectCollectionCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:diagnosisOnEyesSelectCollectionCellId1 forIndexPath:indexPath];
    if (self.eyePositionModelArray.count > 0) {
...
- (void)initBottomView{
    ...
    [nextBtn addTarget:self action:@selector(confirmkRightBtnClick) forControlEvents:UIControlEventTouchUpInside];
    nextBtn.backgroundColor = [UIColor whiteColor];
    [self.view addSubview:nextBtn];
}

- (void)confirmkRightBtnClick{
    if (![[AppStatus sharedInstance] logined]) {
        UserLoginViewController *ulc = [[UserLoginViewController alloc] initWithFrom:account_session_from_user_diagnosisOnRightEyes];
        [self.navigationController pushViewController:ulc animated:YES];
        return ;
    }else{
        NSMutableArray *selectRightPointIdArray = [NSMutableArray new];
        for (EyePositionModel *model in self.selectModelArray) {
            [selectRightPointIdArray addObject:@(model.positionId)];
        }
        DiagnosisEyesResultController *dervc = [[DiagnosisEyesResultController alloc] initWithSelectLeftIdArray:self.pointIdArray rightIdArray:selectRightPointIdArray];
        [self.navigationController pushViewController:dervc animated:YES];
    }
}
