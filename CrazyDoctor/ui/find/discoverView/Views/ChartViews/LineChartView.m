...
- (void)drawRect:(CGRect)rect {
    ...
    NSLog(@">>>>>>>>>>>纵坐标的最大值>>>>>>>>>>>%d----%d",maxValue,(self.targetNum));
    if ((self.targetNum) <= maxValue) {
        
        float y = self.frame.size.height - (MARGIN_TOP + (self.targetNum)*(self.dashedSpace/self.yNumberSpace));
        
        UILabel *label = [[UILabel alloc] init];
        label.center = CGPointMake(MARGIN_LEFT/2, y);
        label.bounds = CGRectMake(0, 0, MARGIN_LEFT - 5, 15);;
        label.textAlignment = NSTextAlignmentCenter;
        label.backgroundColor = [UIColor clearColor];
        label.textColor = [UIColor whiteColor];
        label.font = [UIFont systemFontOfSize:10];
        label.textAlignment = NSTextAlignmentRight;
        label.text = [NSString stringWithFormat:@"%.1fk",((float)self.targetNum/1000)];
        [self addSubview:label];

        UIView *targetView = [[UIView alloc] initWithFrame:CGRectMake(MARGIN_LEFT, y, screen_width-75, splite_line_height)];
