...
+(NSMutableArray*)IndexArray:(NSArray*)stringArr
{
    ...
    {
//        NSLog(@">>>>>>>>>>>>>>>>此处的拼音是什么>>>>>>>>>>>>>>>>>>>>>>%@",((ChineseString*)object).pinYin);
...
//    NSLog(@"-------/获取字符串中文字的拼音首字母并与字符串共同存放-------------------%@",stringArr);
    ...
    for(int i=0;i<[stringArr count];i++)
    {
        ...
        //此方法存在一些问题 有些字符过滤不了 字符串过滤有问题 会把字符串也会过滤掉
       // NSCharacterSet *set = [NSCharacterSet characterSetWithCharactersInString:@"@／：；（）&yen;「」＂、[]{}#%-*+=_\\|~＜＞$€^•'@#$%^&*()_+'\""];
        //11.1
        NSCharacterSet *doNotWant = [NSCharacterSet characterSetWithCharactersInString:@"[]{}（#%-*+=_）\\|~(＜＞$%^&*)_+ "];
        chineseString.string = [chineseString.string stringByTrimmingCharactersInSet:doNotWant];
